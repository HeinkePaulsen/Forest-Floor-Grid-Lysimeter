...
void setup() {
   ...
   TEST_SYSTEM();
   DateTime now = rtc.now();
   sprintf(filename, "%04d%02d%02d.csv", now.year(),now.month(),now.day());  //Filename for data file is defined
 
   File dataFile = SD.open(filename, FILE_WRITE);
     dataFile.println("Wasserhaushalt");
     dataFile.println("Date;Time;Scale_1;Scale_2;Scale_3;Scale_4;Total;TB_1;EC_1;Temp_1;TB_2;TB_3;TB_4");   //an aufgenommene Daten anpassen
     dataFile.close();
   }
...
void SETUP_HX711()            //Kalibrierung je nach Seriennummer wird hier die spezifische Kalibrierung durchgeführt
{
  ...
  SET_MUX16(0);
  delay(5);
  scale1.set_scale(297.31);              //hier muss der Wert aus der Kalibrierung eingesetzt werden
  scale1.tare();
  Serial.print("Channel 1: ");
  fa = scale1.get_units(5);
  Serial.println(fa,0);
  ...
  SET_MUX16(1);
  delay(5);
  scale2.set_scale(295.84);              //hier muss der Wert aus der Kalibrierung eingesetzt werden
  scale2.tare();
  Serial.print("Channel 2: ");
  fb = scale2.get_units(5);
  Serial.println(fb,0);
  ...
  SET_MUX16(2);
  delay(5);
  scale3.set_scale(301.07);              //hier muss der Wert aus der Kalibrierung eingesetzt werden
  scale3.tare();
  Serial.print("Channel 3: ");
  fc = scale3.get_units(5);
  Serial.println(fc,0);
...
void COLLECT_DATA()
{
  ...
  File dataFile = SD.open(filename, FILE_WRITE);
    ...
    dataFile.print(";");
    dataFile.print(time.timestamp(DateTime::TIMESTAMP_TIME));          //hier wird das aAtum geschrieben
    dataFile.print(";");
    dataFile.print(fa, 0); dataFile.print(";");
    dataFile.print(fb, 0); dataFile.print(";");
    dataFile.print(fc, 0); dataFile.print(";");
    dataFile.print(fd, 0); dataFile.print(";");
    dataFile.print(fa+fb+fc+fd,0);  dataFile.print(";");            //hier werden die Gewichte geschrieben
    
    Serial.println("Read Grid 1");
    dataFile.print(Kipp[0],0);                                        // count TB_1
    dataFile.print(";");
    Kipp[0] = 0; 
    dataFile.print(Read_EC1());                                       // read EC_1
    dataFile.print(";");
    dataFile.print(Read_Temp1());                                   // read temp_1
    dataFile.print(";");

    Serial.println("Read Grid 2");
    dataFile.print(Kipp[1],0);                                        // count TB_2
    dataFile.print(";");
    Kipp[1] = 0;

    Serial.println("Read Grid 3");
    dataFile.print(Kipp[2],0);                                        // count TB_3
    dataFile.print(";");
    Kipp[2] = 0;

    Serial.println("Read Grid 4");
    dataFile.println(Kipp[3],0);                                        // count TB_4
   // dataFile.print("  ");
    Kipp[3] = 0;
...
int Read_Temp1() {
  SET_MUX16(9);
  delay(1);
  float tmp = map(analogRead(ANALOG_READ_PIN), 0, 544 , -40, 125);       // Berechnung des Float-Werts
  int tempInt = (int)tmp;                                                // Umwandlung des Float-Werts in einen Integer-Wert
